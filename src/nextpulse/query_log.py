...
`session_id` are PERSONAL DATA. The `question` and `standalone_query` columns
may also contain PII typed by users (names, fiscal codes, IBANs, etc.) — they
are stored pre-masked (see api.py) but treated as PII for retention purposes.
...
`user_id`, `session_id`, `question`, and `standalone_query` to NULL. The
residual data stays available for aggregate statistics but is no longer linked
to an individual or their query text, so it exits the GDPR perimeter.
...
CREATE TABLE IF NOT EXISTS query_log (
    ...
    role             TEXT,
    question         TEXT,                    -- PII · NULLed on anonymization
    standalone_query TEXT,                    -- PII · NULLed on anonymization
    confidence       TEXT,
    grounded         INTEGER,
    ambiguous        INTEGER,
    top_score        REAL,
    n_sources        INTEGER,
    model            TEXT,
    anonymized_at    TEXT                     -- set when all PII columns cleared
);
...
class QueryLog:
    ...
    def _migrate(self) -> None:
        ...
        with self._connect() as conn:
            for col, defn in [("pii_masked", "INTEGER"), ("latency_ms", "INTEGER")]:
                try:
                    conn.execute(f"ALTER TABLE query_log ADD COLUMN {col} {defn}")
                except Exception:
            ...
            cur = conn.execute(
                """INSERT INTO query_log
                       (created_at, user_id, session_id, role, question,
                        ...
                        top_score, n_sources, model, pii_masked, latency_ms)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (
    ...
    def anonymize_older_than(
        ...
        """NULL all PII columns on rows older than `months`. Returns rows changed.

        NULLed: user_id, session_id, question, standalone_query (all may carry PII).
        Rows are not deleted. Idempotent: rows where all four columns are already
        NULL are skipped, so re-running on the same window changes 0 rows.
        """
            ...
            cur = conn.execute(
                """UPDATE query_log
                      SET user_id = NULL, session_id = NULL,
                          question = NULL, standalone_query = NULL,
                          anonymized_at = ?
                    WHERE created_at < ?
                      AND (user_id IS NOT NULL OR session_id IS NOT NULL
                           OR question IS NOT NULL OR standalone_query IS NOT NULL)""",
