...
_DEFAULT_RULES: List[_Rule] = [
    ...
    _Rule("CREDIT_CARD", re.compile(r"\b\d{4}[- ]\d{4}[- ]\d{4}[- ]\d{4}\b")),
...
class MaskingSession:
    ...
    def unmask(self, text: str) -> str:
        ...
        # Pattern targets only PII token format [ENTITY_N] to avoid colliding with
        # source citations like [Fonte: file.pdf, pag. 2] in the LLM output.
        if "[" in text:
